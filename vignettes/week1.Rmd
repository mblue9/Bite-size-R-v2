...
> * Edit a precreated report using R
...
As this is a 'next-steps' course, designed for follow-on practice with cancer datasets after completing a general introductory R course, we assume you know some R basics. [R for Data Science book](https://r4ds.had.co.nz/index.html) is an excellent reference book if you want to look up anything you don't recall or are not very familiar with. We also assume you know some reasons why learning R is good, as you've already started learning, but below are a list of reasons why learning R is useful and worth your time. 
...
By default the top left-hand pane is one for creating, editing & running R scripts and reports.
...
perform only one role in your analysis workflow and so you may have several R scripts which you call, in a particular sequence, to analyse your data. A script is basically a text file that contains R commands and (ideally) comments to explain what the codes function is (as a documentation process).
 ...
 * Download the example report file in the link above
 * In your RStudio top menu go to File > Open File and open the example report file 
 * Click the Knit button to generate the report
 * Try out the exercise to edit the file.

Next we'll explain some of the things you can see in the file.
...
When importing the METABRIC dataset we specified the file using a URL `https://zenodo.org/record/6450144/files/brca_metabric_clinical_data.tsv`
as it's located on the web. If you have a file on your computer you can specify the path to where it is located. For example, if we paste the URL above into our web browser we can download the file and then import it.

```{r eval = FALSE}
# assumes file is in our Downloads folder, "~" means our home directory e.g./home/maria
metabric_downloaded <- read_tsv("~/Downloads/brca_metabric_clinical_data.tsv")
```
...
If you're working with an R Markdown report (Rmd file) your working directory will be the directory in which the report is located. In this case you will probably want to read files from
locations that are relative to the directory in which you run the report.

If you're writing an R report in RStudio, you can set the working directory to be
the same directory as that which contains your file using the menu:
...
Alternatively (and better) you can create an RStudio Project (File > New Project) as described [here](https://r4ds.had.co.nz/workflow-projects.html#rstudio-projects) to automatically keep track of and set the working directory.
...
One final handy tip: summing the result of `is.na()` will count the number of missing values in a vector. If you don't recall what a vector is, [see here](https://r4ds.had.co.nz/vectors.html).

```{r}
sum(is.na(metabric$`Mutation Count`))
```

Can you see why this works? *(hint: look at the output of `is.na(metabric$`Mutation Count`)`, use '`?sum` to check what kind of vector `sum()` expects
